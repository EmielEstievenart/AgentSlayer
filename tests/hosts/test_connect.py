...
def host(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> ScriptedHost:
    ...
    monkeypatch.setattr("agentclip.hosts.ssh.SshHost", build)
    monkeypatch.setattr(
        "agentclip.config.default_global_config_path", lambda: tmp_path / "no-global.toml"
    )
    monkeypatch.setattr(
        "agentclip.hosts.connect.default_remote_state_dir",
...
def test_a_refused_dial_fails_the_connect_step(
    ...
    monkeypatch.setattr("agentclip.hosts.ssh.SshHost", lambda *a, **kw: dead)
    monkeypatch.setattr(
        "agentclip.config.default_global_config_path", lambda: tmp_path / "no-global.toml"
    )
...
def test_a_host_key_decline_aborts_the_attempt(
    ...
    monkeypatch.setattr("agentclip.hosts.ssh.SshHost", lambda *a, **kw: refused)
    monkeypatch.setattr(
        "agentclip.config.default_global_config_path", lambda: tmp_path / "no-global.toml"
    )
...
def test_the_advertised_attempt_count_is_the_one_ssh_host_enforces() -> None:
    ...
    from agentclip.hosts import ssh
