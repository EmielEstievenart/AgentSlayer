...
* ``/new`` - the user is abandoning the conversation on screen, so the next
  ...
  view calls ``request_new_session`` back once the click has landed, so a
  browser that could not be clicked keeps the session on the chat it still has.
...
async def test_new_opens_a_fresh_browser_chat_immediately(
    ...
    paste - and the reset follows the click that landed."""
...
async def test_new_resets_nothing_when_the_chat_could_not_be_opened(
    controller: SessionController, view: FakeChatView
) -> None:
    """A refused click leaves the old conversation on screen, so the session that
    owns it has to stay too - otherwise the next turn pastes into a chat the
    fresh session knows nothing about."""
    await start_session(controller, view)
    ...
    assert view.new_chats_opened == 1  # it was asked for...
    assert view.cleared == 0  # ...and nothing was thrown away when it failed
