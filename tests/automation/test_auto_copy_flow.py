...
``agentclip.screen`` behind one object - so the fixture below patches
``agentclip.automation.ops``' own names, which is the use site the default
implementation calls through. Everything the sequences still have to ASK a shell
is :class:`~agentclip.automation.host.AutomationHost`, and ``FakeHost`` is a
...
import agentclip.automation.controller as controller_mod
import agentclip.automation.ops as ops_mod
...
def machine(monkeypatch: pytest.MonkeyPatch) -> Iterator[_Machine]:
    ...
    Patched on ``agentclip.automation.ops`` - the module that imported them from
    ``agentclip.screen`` - because that is the use site the default
