"""The calibration window - the GUI surface that runs where the pixels are.
...
split mode), and the chat GUI opens it beside itself in local mode.

Nothing in this package imports ``agentclip.shell.app`` or
``agentclip.shell.gui.view``: a calibration process has no session, no engine
and no transcript, and the import graph is where that stays true.
"""
