...
This is **not** a licence to grow sections 1–5: the discipline above still stands and the measurement is still hand-made. What the slack buys is room for *optional* sections that only some services carry. Measured at a 12,000-char preset with a saturated skills listing: **11,982 chars** with everything off, **12,906** with `edit_by_lines` on (§3.1 — +924 for the `replace_lines` entry, the `numbered` read_file entry and the §5 ordering rule), against a 13,200 ceiling — **294 chars of slack**, which `extra_instructions` also draws on.
