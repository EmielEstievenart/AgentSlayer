...
Nothing here opens a window: the bridge's sink is a list (tests/shell/chat/conftest.py).
"""
...
def test_a_cancel_from_the_page_reaches_the_controller(harness: Harness) -> None:
    ...
    spy = ControllerSpy()
    harness.view._controller = spy  # type: ignore[assignment]
    # Straight onto the view: the runner's marshalling is tests/shell/chat/test_runner's.
...
def test_the_automation_paints_all_reach_the_page(harness: Harness) -> None:
    ...
    # nothing here (ui-monitor.md 6.4, tests/shell/monitor_ui/).
