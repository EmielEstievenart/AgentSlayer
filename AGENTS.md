...
| **Monitor UI** | the Monitor's own window: Serve panel, service editor, ELEMENTS, region picker, `/identify` | `src/agentclip/shell/monitor_ui/` | `agentclip-monitor` (the Chat UI also opens it in-process in local mode; `agentclip --calibrate` is a stub that names the binary) |
