...
May import ``agentclip.screen``, ``agentclip.clip`` and ``agentclip.config``
(the OS seams the loop is made of); must never import ``textual``,
``agentclip.app`` or ``agentclip.tui`` - a shell may depend on the automation,
