...
(first child, above PROJECT: Static id=side-paste-flash — the blinking ">>> PRESS CTRL+V <<<" banner, hidden until an outbound copy; §3.4b)
```
...
- **The paste flash**: automation covers browser→AgentClip, but AgentClip→browser still ends in a human keystroke — the outbound payload sits on the clipboard until the user presses Ctrl+V in the chat. So `copy_outbound`, right after its focus click, turns on `#side-paste-flash`: a deliberately obnoxious banner at the very top of the sidebar (`>>> PRESS CTRL+V <<<`, bold, red/yellow, blinking at 0.4 s via a Sidebar-owned timer toggling the `flash-alt` class — pure presentation, so the dumb widget may own it). It hides when the moment has provably passed: a `MATCH` busy probe (the model is chewing — the paste landed), a new `ClipboardCaptured` (the conversation moved on without it), or `clear_transcript()`. `Sidebar.show_paste_flash`/`hide_paste_flash` are the only entry points; display on/off is the tested contract.
