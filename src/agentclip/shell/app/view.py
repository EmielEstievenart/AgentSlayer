...
class ChatView(Protocol):
    ...
    # What the model SAID to the user: one ===CLIP:SAY block, markdown, meant to
    # be read. ``add_prose`` stays for the text a model leaves outside every
    # block - tolerated, shown, but nothing the protocol asked for.
    ...
    # The brackets around ONE INGESTED REPLY, and the reason they exist: a reply
    # is not one transcript event but several - the prose, then a widget per
