...
- the chat shell has exactly one (``shell/chat/shell.py``) - so nothing here is a
...
# The asset files this window needs, resolved the way the Chat UI's window
# resolves its own: ``importlib.resources`` against the PACKAGE, never
# ``__file__``, so a frozen build finds them under the onefile extraction. That
# only works if ``packaging/agentclip.spec`` collects this directory at the
# package-relative path, which it does beside the Chat UI's (gui.md §5). The
# resolving itself is ``webview/assets.py``'s, shared by both windows.
ASSET_PACKAGE = "agentclip.shell.monitor_ui"
...
class CalibrationBridge(Bridge):
    ...
    identical for both windows and is already pinned by ``tests/shell/chat``.
    ...
    ``editor`` (the whole :class:`~agentclip.shell.webview.service_editor.ServiceEditor`
...
def asset_dir() -> AbstractContextManager[Path]:
    """This window's packaged ``assets/`` directory, for as long as it is used."""
    return package_asset_dir(ASSET_PACKAGE)


def entry_url(assets: Path) -> str:
    """The ``file://`` URL for this window's page, carrying the app version."""
    return page_url(assets)
