...
| `skill` | `name`\* | A short header — the skill's absolute `folder:`, and a bounded `files:` listing of what is bundled beside `SKILL.md` — then a blank line, then the body of the named Agent Skill (a reusable procedure/reference), verbatim. Read-only, auto (no gate). Those side files are readable by absolute path (`read_file`) and runnable via `run_command`; the workspace carries the skill folders as read-only carve-outs. The header rides here and NOT in the catalog, which the bootstrap budget could not afford. Only present when ≥1 model-invocable skill is discovered; the catalog lists each skill's name + description so the model picks one and loads it on demand (progressive disclosure). Unknown name ⇒ `error code=unknown_skill` listing the available skills. See `docs/design/skills.md`. |
