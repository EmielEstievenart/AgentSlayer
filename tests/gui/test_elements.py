...
def test_the_same_pixels_are_encoded_once(
    ...
    encodes: list[int] = []
    monkeypatch.setattr(
        "agentclip.gui.view.element_png",
...
async def test_the_picker_applies_to_the_selected_window_and_rebuilds(
    ...
    spy = PickerSpy(drawn)
    monkeypatch.setattr("agentclip.gui.view.pick_region", spy)
    suspends: list[str] = []
    monkeypatch.setattr(
...
async def test_the_slot_is_frozen_when_the_overlay_opens(
    ...
    monkeypatch.setattr("agentclip.gui.view.pick_region", moving)
...
async def test_drawing_a_window_the_poller_is_not_watching_leaves_it_alone(
    ...
    rebuilds = stub_rebuilds(monkeypatch)
    monkeypatch.setattr(
        "agentclip.gui.view.pick_region", PickerSpy(ScreenRegion(0, 0, 400, 300))
    )
...
async def test_a_cancelled_pick_changes_nothing_and_says_so(
    ...
    view = harness.view
    monkeypatch.setattr("agentclip.gui.view.pick_region", PickerSpy(None))
    await view._pick_chat_region(AgentSlot.MASTER)
    assert view.automation.calibration(AgentSlot.MASTER).chat_region is None
...
async def test_a_failed_picker_is_an_error_toast_not_a_crash(
    ...
    view = harness.view
    monkeypatch.setattr("agentclip.gui.view.pick_region", PickerSpy(error="the picker died"))
    await view._pick_chat_region(AgentSlot.MASTER)
...
async def test_identify_captures_first_then_draws_and_summarises(
    ...
    monkeypatch.setattr(
        "agentclip.gui.view.capture_region",
    ...
    monkeypatch.setattr(
        "agentclip.gui.view.identify_elements",
        lambda *args, **kwargs: (order.append("identify"), found)[1],
    )
    monkeypatch.setattr(
        "agentclip.gui.view.draw_identify_overlay",
...
async def test_identify_searches_with_the_pollers_own_settings(
    ...
    monkeypatch.setattr(
        "agentclip.gui.view.capture_region", lambda _r: bgrx([(0, 0, 0)] * 16, 4, 4)
    )
    ...
    monkeypatch.setattr("agentclip.gui.view.identify_elements", spy_identify)
    monkeypatch.setattr("agentclip.gui.view.draw_identify_overlay", lambda elements: None)
...
async def test_a_failed_overlay_resumes_the_detectors_anyway(
    ...
    monkeypatch.setattr("agentclip.gui.view.draw_identify_overlay", boom)
    view._picker_open = True
    await view._identify_live_window()
