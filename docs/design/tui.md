...
The REASONING block is a live "is the model still generating?" readout, independent of the chat region above it, and it can be fed by up to three independent detectors whose verdicts fold into one combined answer (§3.4b). The user presses "Set busy region...", draws a box around the chat UI's busy/stop indicator **while the model is generating**, and that drawn region's pixels are captured as a calibration baseline (`agentclip.screen.capture.capture_region`). MainScreen then polls the same region every `_BUSY_POLL_S` seconds (`agentclip.screen.busy.probe_busy`) and compares each fresh capture against the baseline: still matching means the model is still reasoning, a changed region means the response finished, and a capture failure is reported as an error. Each poll repaints `#side-busy` via `Sidebar.update_busy` with an unmistakable state string (`● GENERATING · match (diff 1.2%)` / `○ response ready · changed (diff 34.0%)` / `✗ capture failed`). "Set idle button..." calibrates the mirror image — an element that looks *different* while generating (the send button is the usual pick), captured while the chat is **idle** — so `#side-idle` reads the inverse polarity: a match means finished, a change means generating.

"Set response region..." (`#set-stale-btn` → `#side-stale`) adds a third, service-agnostic angle, for chats whose busy/idle pixel cues are unreliable or absent: the user draws a box around the response text itself, no baseline capture happens at calibration time, and `agentclip.screen.stale.StaleTracker` instead compares each poll's capture to the *previous* poll's — "no reasoning happening" reads as "no pixel changes at all," which needs no particular cue to trust. Once the region has read unchanged (`STALE_MAX_DIFF = 0.002` over up to `STALE_MAX_SAMPLES = 16384` samples) for `required_ticks` consecutive polls it reports STALE (finished); the ticks come from the active service preset's `stable_seconds` (§1.4), converted at poller start via `max(1, round(stable_seconds / _BUSY_POLL_S))`. `#side-stale` repaints the same way (`○ response ready · stale (still ×4)` / `● GENERATING · changing (diff 0.08% · still ×2)` / `✗ capture failed`). Any subset of the three detectors may be calibrated; whichever are, they also drive the auto-copy-click trigger described in §3.4b — nothing else consumes any of their verdicts. Calibrations are scoped like the chat region — they describe where the service's windows are, not what one conversation said — so all three **survive** `/new` and the summary's *new session*: only the live-slot pointer resets to MASTER, and the detector poller restarts against the surviving baselines/regions rather than dropping them.
...
- **Scope**: app-run-scoped. Every calibration lives in a `SlotCalibration` on MainScreen (§1.6), shows in the sidebar's `#side-region` / `#side-chatbox-*` labels, and can be redrawn mid-session (window moved). Calibrations describe where the service's windows are, not what one conversation said, so they **survive** `/new` / the summary's *new session* for both slots — a new conversation in the same windows needs no re-drawing (and a sub-agent slot calibrated mid-session actually enables `delegate` on the next `/new`, which is the advertised workflow). Only the pointers (`_calibrating`, `_live`) go home to MASTER, and the detector worker restarts against the surviving live-slot baselines. Nothing is persisted to config: closing AgentClip forgets everything.
...
- **Arming and firing**: each poller tick posts up to three messages in a **fixed order** — `BusyProbed` → `IdleProbed` → `StaleProbed` (`tui/messages.py`) — skipping whichever detector isn't calibrated for the live slot. `_start_detector_worker` (re)builds the poller, and a fresh `StaleTracker` when `stale_region` is calibrated, whenever a detector is (re)calibrated or the live slot moves; the tracker's `required_ticks` comes from the *active* service preset's `stable_seconds` (§1.4) via `max(1, round(stable_seconds / _BUSY_POLL_S))` at the poller's 0.5 s cadence. Each handler — `on_busy_probed` / `on_idle_probed` / `on_stale_probed` — records its detector's verdict and calls `_evaluate_finish` only when `_finish_tick_closed_by` says its message is the tick's **last** given what is calibrated (busy closes the tick when nothing later is calibrated, idle when stale isn't, stale always): this is what lets `_evaluate_finish` fold the combined verdict exactly once per tick no matter which subset of the three detectors is calibrated, instead of acting on a half-reported tick. `_evaluate_finish` folds every detector that has ever reported (`_seen`) into one verdict: **any** `False` (generating) arms the trigger and resets the streak; only once **every** live verdict reads `True` (finished) does the streak advance, and reaching **2** consecutive all-`True` ticks fires `_auto_copy_flow` and disarms — so with several detectors calibrated, that agreement is the whole point of having more than one. A capture error (`None`) breaks the streak but leaves the arm alone — a single bad frame must not cancel an in-flight finish. Evaluation is suspended for the flow's entire run (`_flow_running`), and the live `StaleTracker` is reset in the flow wrapper's `finally` (`_run_auto_copy_flow`): the flow itself scrolls and hover-scans the very response region the stale detector watches, so without the suspension and the reset it would read its own mouse work as a fresh generation and re-arm/re-fire itself forever.
...
- **Scope and layering**: the calibration (`_copy_region`, `_copy_template`) survives `/new` like every other calibration (§3.4a); only the trigger state (`_copy_armed`, `_copy_changed_streak`) is reset by `clear_transcript()`. Nothing persisted to config. Lives entirely on MainScreen, like the focus click — the controller never knows the feature exists.
