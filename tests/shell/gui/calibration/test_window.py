...
the ``--calibrate`` dispatch - the same three things ``tests/shell/gui/test_shell.py``
...
from agentclip.shell.gui.calibration.window import (
    ASSET_DIR,
    ASSET_NAMES,
    ASSET_PACKAGE,
    ENTRY_PAGE,
...
SPEC = Path(__file__).resolve().parents[4] / "packaging" / "agentclip.spec"
...
def test_the_spec_ships_the_page_where_importlib_resources_will_look() -> None:
    ...
    or ``files("agentclip.shell.gui.calibration")`` finds nothing under
    ``_MEIPASS``."""
    text = SPEC.read_text(encoding="utf-8")
    assert '"agentclip/shell/gui/calibration/assets"' in text
    assert "CALIB_ASSETS" in text
...
def test_calibrate_opens_the_window_and_builds_no_engine(
    ...
    monkeypatch.setattr(
        "agentclip.shell.gui.calibration.window.run_calibration", calibrate
    )
...
def test_calibrate_refuses_a_project_that_is_not_there(
    ...
    monkeypatch.setattr(
        "agentclip.shell.gui.calibration.window.run_calibration",
...
def test_importing_the_window_does_not_import_pywebview() -> None:
    ...
    module = importlib.import_module("agentclip.shell.gui.calibration.window")
