...
├── Static "CHAT WINDOW"                            # the ONLY per-slot block
...
├── Static                 id=side-profile-title    # "APPEARANCE · chatgpt" — the SERVICE's, shared by both slots
├── Button                 id=capture-<kind>-btn    # .capture-btn, one per TemplateKind, "Capture busy indicator..." etc.
├── Static                 id=side-tpl-<kind>       # "not captured" / "24×24 · captured" / "● GENERATING · match (diff 1.2%)"
│   …six of those pairs, in TemplateKind declaration order…
├── Static                 id=side-profile-note     # "4/6 captured · saved for chatgpt"
├── Button                 id=forget-profile-btn    # "Forget these templates" → ConfirmScreen → profile_store.delete_profile
...
The column has exactly **two kinds of block** below the pickers, and the split is the whole design. CHAT WINDOW is per *slot*: the AGENT SLOT picker above it chooses which window `#set-region-btn` writes into, and `Sidebar.show_slot(cal, note)` repaints it in one go. APPEARANCE is per *service*: the same six captures serve both slots, persist to disk, and are repainted by `Sidebar.show_profile(profile)` whenever a capture lands or the service picker moves (`Sidebar.ServiceChanged` — a domain message, because a different service is a different set of appearances, not a display change: MainScreen reloads the profile, repaints, recomputes the readiness note and restarts the detectors). The APPEARANCE block is *generated* per `TemplateKind`, with the kind encoded in each button's id (`capture-<kind>-btn`, class `capture-btn`), so MainScreen routes all six through **one** `@on(Button.Pressed, ".capture-btn")` handler — a seventh appearance is an enum member and nothing else.
...
**Appearance readout** (`#svc-templates`, `#svc-forget-templates-btn`): the editor is where a user reasons about a service, so *"does this one know what its copy button looks like?"* has to be answerable here — the line is re-derived from `profile_store.load_profile(profile_root, key)` on every selection change. The editor deliberately **never captures**: drawing a box needs the browser on screen, which is a main-screen job. It can only forget, behind the shared `ConfirmScreen`, and that is kept separate from "Delete" on purpose — a user whose browser theme changed wants to recapture, not to lose their size settings. Because the editor can delete profiles out from under MainScreen's per-run cache, `MainScreen.update_config` clears that cache and repaints.
...
**Slots — one drawn box per browser window.** A slot was six calibrations; it is now a single rectangle, because the two questions it conflated have different owners. *What does this thing look like?* (the stop button, the copy icon, both chat-box layouts, the new-chat control) is a property of the **service** — identical in every window it is ever opened in — so it lives in a `ServiceProfile` (`agentclip.screen.profile`): captured once, persisted as PNGs, reloaded next run, shared by every slot pointed at that service. *Where should I look for it?* is genuinely per window, and is all `SlotCalibration` (`agentclip.screen.slot` — pure data, stdlib only, unit-tested without Textual) still holds: `chat_region`, one per `AgentSlot`, where `MASTER` is the chat the session runs in and `SUBAGENT` the window a delegated sub-agent gets.

The headline is the setup cost. **A sub-agent window costs exactly one drag**, because it inherits every capture the master already made; and a browser the user moved, resized or dragged to the other monitor costs one *re*-drag instead of six recaptures. Two independent pointers ride on top:
...
Readiness is therefore a function of the **pair**, not a property of either half, so the rules are module-level functions taking `(cal, profile)`: `can_paste`, `can_finish`, `can_copy`, `can_delegate`, `missing`. `can_paste` and `can_finish` are the drawn window alone — the input box is found inside it, and a rectangle that stops changing *is* the staleness finish detector, so **one drag makes a slot both pasteable and finishable**. `can_copy` adds `profile.has(COPY)`; `can_delegate` adds `profile.has(NEW_CHAT)` on top of all of it. It is strict on purpose: without all three pieces a sub-run would strand halfway, so a half-calibrated slot must read as unavailable. `missing()` returns the gaps in calibration order — `MISSING_CHAT_REGION`, `MISSING_COPY`, `MISSING_NEWCHAT` — which the sidebar shows in `#side-slot-note` and which the controller embeds in the error the *model* gets if it calls `delegate` against an uncalibrated host. Losing the window lists all three, which is honest rather than noisy: with nowhere to search, neither button can be found, and one drag closes all three gaps.

Because half the answer is the profile, the "sub-agent slot ready" seam (`_after_calibration`) has to fire on **captures as well as region draws** — capturing a copy button can flip delegation on without any box being drawn, for both slots at once.
...
- Every branch also repaints `#side-tpl-copy` with the captured size plus a short ASCII status (`captured` / `hover-scanning` / `clicked (diff 0.03)` / `click did not take` / `not found` / `capture failed`).
...
**The six kinds** (`agentclip.screen.profile.TemplateKind`, in declaration order — the sidebar's APPEARANCE block is generated from it): `BUSY` (on screen only while the model generates — the stop button, not a spinner: an animation is a different picture every frame and can never be matched back), `IDLE` (its mirror: on screen only while the chat is idle, usually the send button), `CHATBOX_INITIAL` and `CHATBOX_ONGOING` (the input box in its fresh-chat and mid-conversation layouts, captured while empty), `COPY` (one response's copy icon), and `NEW_CHAT` (the browser's new-chat control). Each kind carries its own overlay `prompt` and its own match threshold (`max_diff`: 0.08 for the three crisp icons, 0.10 for the hover-tinted new-chat button, 0.20 for the big mostly-flat chat boxes) — properties of the *appearance*, so they live on the enum, not in the TUI. `ServiceProfile` is the per-service bag of captured kinds; readiness questions (§1.6) and every search go through it.

**The search** (`agentclip.screen.template`): pure Python cannot afford a naive 2D scan (a 1920×1080 region is ~2M candidate offsets), so candidate generation is pushed into C-level `bytes` operations. At capture time, `Template.build` quantises the image's blue plane (`pixels[0::4].translate(>>5 table)`, 8 buckets — a bucket comfortably exceeds the tolerance-24 noise band) and picks 8 eight-byte **anchors** at the most varied windows on distinct rows. Per search, the scene's plane is built the same way once, each anchor's needle is swept with `bytes.find` (capped at 512 candidates per anchor, deduped across anchors), and each surviving candidate origin passes a 16-pixel cheap probe before the full 1024-sample strided verification (per-channel tolerance 24 on B/G/R, the undefined X byte skipped) produces the diff that `max_diff` judges. Quantisation has a boundary hazard — a pixel drifting across a bucket edge breaks an exact `find` — and the 8-anchors-on-different-rows spread is the mitigation: one surviving anchor is enough, and a genuinely static element keeps most of them. Worst case lands around 50–75 ms per search on a 2560×1440 region, a comfortable fraction of the 0.5 s poll tick on a worker thread; the typical busy-path probe (early exit on the first verified match) is ~10 ms. `find_in_region` answers "is it there", `find_lowest_in_region` answers "the newest response's copy button" (max y), and `match_rect` converts a scene-local hit back to absolute screen coordinates for the click.
...
**Invalidation is manual by design.** A profile is per service *and per theme*: switch the chat's light/dark theme or hit a site redesign, and the captures stop matching — the fix is re-pressing the one capture button (the new capture overwrites the old), or "Forget these templates" / the editor's "Forget appearance" to start clean. Nothing expires automatically: a template that stops matching degrades to "not found" toasts and the stale detector still finishes the turn, which is annoying, visible, and safe — the failure mode automation must never have is clicking somewhere wrong quietly.
