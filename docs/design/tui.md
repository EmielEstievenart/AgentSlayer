...
├── Static                 id=side-profile-note     # read-only: "appearance: 4/6 captured · F2 to capture" (Sidebar.profile_summary)
...
├── Static "DETECTION"                              # four read-only lines the running automation writes into
├── Static                 id=side-tpl-busy         # "busy · no verdict yet" / "busy · ● GENERATING · match (diff 1.2%)"
...
- **CHAT WINDOW**, the one per-*slot* block: the AGENT SLOT picker above it chooses which window `#set-region-btn` writes into, and `Sidebar.show_slot(cal, note)` repaints it in one go.
...
- One read-only **appearance summary** under the service picker, `#side-profile-note` — *"appearance: 4/6 captured · F2 to capture"* — so *is this service usable at all?* is answerable at a glance without opening the editor.

`Sidebar.show_profile(profile)` repaints the summary **and** resets the three probe lines, in one call, because both go stale for the same reason: it is called when the service picker moves (`Sidebar.ServiceChanged` — a domain message, because a different service is a different set of appearances, not a display change: MainScreen reloads the profile, repaints, recomputes the readiness note and restarts the detectors) and when `MainScreen.update_config` adopts an editor visit, and in both cases the verdicts on screen belong to detectors that were hunting the *old* service's appearances.
...
Because half the answer is the profile, the "sub-agent slot ready" seam (`_after_calibration`) has to fire on **captures as well as region draws** — capturing a copy button can flip delegation on without any box being drawn, for both slots at once. Since captures moved into the editor (§1.4) that half arrives through `MainScreen.update_config`, which drops the profile cache and re-runs `_after_calibration` for exactly this reason.
...
- **Capturing**: the service editor's "Capture copy button..." (`#svc-capture-copy-btn`, §1.4) spawns the same draw-a-box overlay as the chat region, prompting (from `TemplateKind.COPY.prompt`) for a **tight** box around **one** copy-button icon — "pick the one under the last response, while the page is idle." Only the *pixels* are kept: they are anchored for search (`ServiceProfile.put`) and written to disk under the **service** (`profile_store.save_template`), so the same capture serves both slots and every later run. The drawn rectangle is not stored at all — the icon is found where it actually is, every time. A capture failure is reported and nothing is filed; so is a *save* failure, since the editor keeps no in-memory profile for the run to fall back on. The sidebar shows only whether it is captured at all (`#side-profile-note`, "appearance: 4/6 captured"); the per-kind size lives on `#svc-tpl-copy` in the editor.
