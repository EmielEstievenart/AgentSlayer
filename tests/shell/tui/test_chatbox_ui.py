...
(agentclip.shell.tui.screens.main); the in-region search is monkeypatched too, since
...
async def test_the_ongoing_box_found_in_the_region_wins(
    ...
    async with app.run_test(size=SIZE) as pilot:
        ...
        assert clicks == focus_clicks(aimed_at(ONGOING_BOX))
...
async def test_new_preserves_the_region_and_the_appearances(
    ...
    async with app.run_test(size=SIZE) as pilot:
        ...
        assert clicks == focus_clicks(aimed_at(ONGOING_BOX), aimed_at(ONGOING_BOX))
