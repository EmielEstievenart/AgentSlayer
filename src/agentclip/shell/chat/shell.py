...
own thread) is :mod:`agentclip.shell.chat.runner`; the ports it drives are
:mod:`agentclip.shell.chat.view`.
...
MISSING_WEBVIEW2 = (
    ...
    f" Chat UI renders in. Install the Evergreen runtime from {WEBVIEW2_DOWNLOAD}"
    " (or fall back to the deprecated TUI: run agentclip --tui)."
)

# The asset files the window needs; also what tests/shell/chat checks is really
# shipped, and what --gui-smoke reads back out of a frozen build. They live in
# agentclip/shell/chat/assets/ as package data - hatchling puts every file under
...
# the onefile extraction (docs/design/gui.md 5). The resolving itself is
# webview/assets.py's, shared with the Monitor UI's window.
ASSET_PACKAGE = "agentclip.shell.chat"
...
def asset_dir() -> AbstractContextManager[Path]:
    """This shell's packaged ``assets/`` directory, for as long as it is used."""
...
def run_gui(
    ...
    *,
    ...
    whole new set of the arguments above (``chat/remote.py:RemoteConnect``).
    ...
    with asset_dir() as assets:
        try:
            ...
            print(f"agentclip: the Chat UI could not start: {exc}", file=sys.stderr)
