...
``chat/view.py`` and ``app.js`` is the two things a model cannot do: schedule the
...
name, so ``tests/shell/chat`` monkeypatches this module and never opens an overlay.
"""
...
# the reason every literal in ``chat/view.py`` is: the two shells may not import
