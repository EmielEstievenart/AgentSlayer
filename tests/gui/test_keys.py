...
controller), like every other key in ``tests/gui/test_chrome.py``. What the PAGE
...
window: ``tests/gui/conftest.py``). The source assertions are written to bite:
...
from agentclip.gui.bridge import JsApi, JsCalls
from agentclip.gui.runner import GuiRunner
from agentclip.gui.view import QUIT_BODY, QUIT_TITLE, THEME_CHOICES, GuiView
...
def test_an_unsavable_theme_still_applies(harness: Harness, tmp_path: Path) -> None:
    ...
    import agentclip.gui.view as view_module
...
def test_the_catalogue_names_the_two_new_event_families() -> None:
    doc = __import__("agentclip.gui.bridge", fromlist=["x"]).__doc__ or ""
